...
import { Post, addComment, likePost, deletePost } from "../../Store/Post-Slice"
...
const PostComponent: React.FC<Post> = ({ post }) => {
    ...
    /*const addCommentController = (id: number) => {
        ...
        const addCommentController = (id : number) => {
            const username = userDetails.username;
            if (commentRef.current) {
                const comment = commentRef.current.value.trim(); // Remove leading/trailing whitespace
                if (comment) { // Check if the comment is not empty
                    const commentData = { username, comment };
                    dispatch(addComment({ comment: commentData, id }))
                    commentRef.current.value = ""; // Clear input field after dispatch
                } else {
                    console.warn("Comment cannot be empty."); // Optional: Log a warning or show an error message
                }
            }
        };
        ...
        <div className="feed-card" key={post.id}>
            ...
            <div className="likes">
                ...
                {post.username === userDetails.username ?
                    <span>
                        <MdDelete size={25} onClick={() => handleDelete(post.id)}/>
                    </span>  : ""
                }
            </div>
            {showComment &&
